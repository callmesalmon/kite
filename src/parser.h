#ifndef KITE_PARSER_H
#define KITE_PARSER_H
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
#include "token.h"
...
ptr_list_t parse_cargs(parser_t *p) 
{
...
node_t *parse_suffix(parser_t *p)
{
	...
	for(;;)
	{
		...
		{ 
			...
			dparse_printf("VARIABLE VALUE: '%s'\n\n", ((node_var_t*)a)->value);
...
node_t *parse_expr(parser_t *p)
{
	...
	if(tok.type == tt_var)
	{
		...
		if(strcmp(tok.value, "if") == 0)
		{
			...
			{
				free_node(cond);
				return parser_error(p, "expected 'do'");
			}
